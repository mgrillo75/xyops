	...
	renderActiveJobs() {
		...
		html += this.getPaginatedGrid( grid_args, function(job, idx) {
			var tds = [
				...
				'<div id="d_vg_jt_elapsed_' + job.id + '">' + self.getNiceJobElapsedTime(job, true, false) + '</div>',
				'<div id="d_vg_jt_progress_' + job.id + '">' + self.getNiceJobProgressBar(job) + '</div>',
				'<div id="d_vg_jt_remaining_' + job.id + '">' + self.getNiceJobRemainingTime(job, true, true) + '</div>',
	...
	handleStatusUpdateView(data) {
		...
		if (data.jobsChanged) {
			...
			jobs.forEach( function(job) {
				...
				div.find('#d_vg_jt_elapsed_' + job.id).html( self.getNiceJobElapsedTime(job, true, false) );
				div.find('#d_vg_jt_remaining_' + job.id).html( self.getNiceJobRemainingTime(job, true, true) );
