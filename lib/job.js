...
class Jobs {
	...
	launchJob(job, callback) {
		...
		// process-launch fields are only trusted after plugin/config hydration below
		this.deleteJobLaunchContext(job);
		...
		// ensure active queue limit also has a job limit
		var qlimit = Tools.findObject(job.limits, { type: 'queue', enabled: true });
		if (qlimit && qlimit.amount && !Tools.findObject(job.limits, { type: 'job', enabled: true })) {
			return callback( new Error("The 'Max Queue Limit' requires a 'Max Concurrent Jobs' limit to also be present." ) );
		}
		...
		if (job.params) {
			...
			// process keys in specific order: event fields first, then the rest
			// workaround for issue # 312
			var pkeys = event.fields ? event.fields.map( field => field.id ) : [];
			pkeys = pkeys.concat( Object.keys(job.params) );
			pkeys = [...new Set(pkeys)]; // dedupe
			
			pkeys.forEach( function(key) {
				if (typeof(job.params[key]) == 'string') {
					job.params[key] = self.messageSub( job.params[key], context );
				}
				
				// allow special prefixed params to override top-level props
				if (key.match(/^_xy_override_(\w+)$/) && job.params[key] && job.params[key].length) {
					var sub_key = RegExp.$1;
					if (self.isReservedJobOverrideKey(key)) {
						self.logJob(5, "Ignoring reserved job override param: " + key, { job: job.id, key: sub_key });
						return;
					}
					self.logJob(9, "Overriding job property with param: " + key, { key: sub_key, value: job.params[key] } );
					job[sub_key] = job.params[key];
				}
			} ); // foreach ordered key
		} // job.params
		...
		// move some things to details so they aren't synced to all users every second
		if (job.input) {
			info.input = job.input;
			delete job.input;
		}
		if (job.workflowData) {
			info.workflowData = job.workflowData;
			delete job.workflowData;
		}
	...
	monitorJobs() {
		...
		Object.values(this.activeJobs).forEach( function(job) {
			...
			if (job.state == 'queued') {
				// build up a job queue id using same logic as findSimilarJobs
				var parts = [ job.event ];
				if (job.workflow && job.workflow.node) parts = [ job.workflow.node ]; // replace event
				if (job.workflow && job.workflow.job) parts.push( job.workflow.job );
				if (job.type) parts.push( job.type );
				var queue_id = parts.join('-');
		...
		for (var queue_id in queues) {
			...
			// use limit definition from first queued job
			// (it SHOULD match the rest of the jobs in the queue group)
			var job = queued[0];
			...
			if (job_limit && (jobs.length < job_limit.amount)) {
				// we have room!  launch queued job waiting the longest (also priority ones first)
				queued.sort( function(a, b) {
					var pa = a.priority ? 1 : 0;
					var pb = b.priority ? 1 : 0;
					if (pa != pb) return pb - pa; // priority first
					return a.started - b.started; // then older first
				} );
				job = queued.shift();
				...
				// FUTURE NOTE: This will only launch a max of 1 queued job per group per second (per tick)
				// However, monitorJobs is also called when jobs complete, so that compensates
			}
		} // foreach queue
	}
		...
		switch (job.state) {
			...
			case 'ready':
				...
				this.runJobActions(job, 'start', function() {
					...
					// if job was suspended / resumed, we may need to re-choose a server
					if (!job.server && !self.chooseJobServer(job)) return;
					...
					socket.send('launch_job', {
						...
						details: Object.assign({}, self.jobDetails[ job.id ], {
							...
							versions: {
								xyops: self.server.__version,
								xysat: server.info.satellite
							}
						}),
					...
					// remove workflowData from jobDetails, so job can send an update (with the same prop name) at completion
					// (this is so it won't get clobbered in the combo_job merge)
					delete self.jobDetails[ job.id ].workflowData;
	...
	findSimilarJobs(job, criteria) {
		...
		// include job type if applicable
		if (job.type) criteria['type'] = job.type;
		
		// for wf nodes use the node id as criteria (replace event)
		if (job.workflow && job.workflow.node) {
			criteria['workflow.node'] = job.workflow.node;
			delete criteria['event'];
		}
		
		// if running inside a workflow, include the parent workflow job id itself as criteria,
		// so that limits are applied in a "single workflow execution run scope" (see gh issue # 218)
		if (job.workflow && job.workflow.job) {
			criteria['workflow.job'] = job.workflow.job;
		}
			...
			if (jobs.length >= job_limit.amount) {
				...
				if (queue_limit && queue_limit.amount) {
					...
					if (queued.length < queue_limit.amount) {
						...
						job.state = 'queued';
						job.position = job.priority ? 0 : (queued.length + 1); // updated every time a job changes state
	...
	filterServerByMaxJobs(job_weight, server_id) {
		...
		// calculate total job weight of all running jobs on the server
		var total_weight = 0;
		this.findActiveJobs({ server: server_id }).forEach( function(job) {
			var job_limit = Tools.findObject( job.limits || [], { type: 'job', enabled: true } ) || { weight: 1 };
			total_weight += (job_limit.weight || 1);
		} );
		
		return job_weight + total_weight <= max_jobs;
	}
	...
	checkAvailableJobServer(job) {
		...
		// convert CSV into array if needed
		if (typeof(job.targets) == 'string') job.targets = job.targets.split(/\,\s*/);
		...
		// too many concurrent jobs may remove server from candidates
		var job_limit = Tools.findObject( job.limits || [], { type: 'job', enabled: true } ) || { weight: 1 };
		var job_weight = job_limit.weight || 1;
		server_ids = server_ids.filter( this.filterServerByMaxJobs.bind(this, job_weight) );
	...
	chooseJobServer(job) {
		...
		this.logJob(9, "Choosing server for job: " + job.id);
		...
		// de-dupe
		server_ids = [ ...new Set(server_ids) ];
		this.logJob(9, "Initial target servers to choose from: " + server_ids.length, { job: job.id, server_ids } );
		
		// filter by actual online servers at the present moment
		server_ids = server_ids.filter( function(server_id) {
			return self.servers[server_id] && self.servers[server_id].enabled;
		} );
		this.logJob(9, "Filtered by online and enabled: " + server_ids.length, { job: job.id, server_ids } );
		
		// certain alerts being active may remove server from candidates
		server_ids = server_ids.filter( this.filterServerByAlerts.bind(this) );
		this.logJob(9, "Filtered by active alerts: " + server_ids.length, { job: job.id, server_ids } );
		
		// too many concurrent jobs may remove server from candidates
		var job_limit = Tools.findObject( job.limits || [], { type: 'job', enabled: true } ) || { weight: 1 };
		var job_weight = job_limit.weight || 1;
		server_ids = server_ids.filter( this.filterServerByMaxJobs.bind(this, job_weight) );
		this.logJob(9, "Filtered by server maxJobs: " + server_ids.length, { job: job.id, weight: job_weight, server_ids } );
		...
		if (job.expression) try {
			...
			this.logJob(9, "Filtered by job expression: " + job.expression + ": " + server_ids.length, { job: job.id, server_ids } );
		}
		catch (err) {
			this.logError('job', "Failed to evaluate job expression: " + job.expression + ": " + err);
		}
		...
		if (!server_ids.length) {
			...
			if (queue_limit && queue_limit.amount) {
				...
				if (queued.length < queue_limit.amount) {
					...
					this.appendMetaLog(job, "Moving job state from {" + job.state + "} to {queued}");
					job.state = 'queued';
					job.position = job.priority ? 0 : (queued.length + 1);
					return false; // stop processing job launch
				}
				else {
					// queue is full
		...
		switch (job.algo) {
			...
			case 'prefer_first_natural':
				server_id = server_ids[0];
			break;
			
			case 'prefer_last_natural':
				server_id = server_ids[ server_ids.length - 1 ];
			break;
			
			case 'prefer_first':
				// prefer alphabetical first
				server_ids.sort( function(a, b) {
					return (self.servers[a].title || self.servers[a].hostname).toLowerCase().localeCompare( (self.servers[b].title || self.servers[b].hostname).toLowerCase() );
				} );
				server_id = server_ids[0];
			break;
			
			case 'prefer_last':
				// prefer alphabetical last
				server_ids.sort( function(a, b) {
					return (self.servers[a].title || self.servers[a].hostname).toLowerCase().localeCompare( (self.servers[b].title || self.servers[b].hostname).toLowerCase() );
				} );
				server_id = server_ids[ server_ids.length - 1 ];
			...
			case 'fewest_jobs':
				var fewest_jobs = 999999;
				server_ids.forEach( function(id) {
					var num_jobs = self.findActiveJobs({ server: id }).length;
					if (num_jobs < fewest_jobs) { server_id = id; fewest_jobs = num_jobs; }
				} );
			break;
	...
	updateJobData(data) {
		...
		Object.values(data).forEach( function(updates) {
			...
			if (updates.markdown && updates.markdown.content) {
				...
				updates.html.content = self.messageSub(updates.html.content, context, "n/a", null);
			...
			else if (updates.html && updates.html.content) {
				updates.html.content = self.messageSub(updates.html.content, context, "n/a", null);
			}
			...
			if (updates.workflowData) {
				info.workflowData = updates.workflowData;
				delete updates.workflowData;
			}
			if (updates.serverData) {
				info.serverData = updates.serverData;
				delete updates.serverData;
			}
	...
	finishJob(job) {
		...
		if (job.ephemeral && info.files && info.files.length) {
			this.appendMetaLog(job, "Removing ephemeral flag, as job has output files.");
			delete job.ephemeral;
		}
		
		// if job has non-zero code, remove ephemeral flag
		if (job.ephemeral && job.code) {
			this.appendMetaLog(job, "Removing ephemeral flag, as job has a non-zero code.");
			delete job.ephemeral;
		}
		...
		this.prepJobLog( job, function() {
			...
			self.runJobActions(job, conditions, function() {
				...
				// apply tag limit
				var tag_limit = Tools.findObject( job.limits || [], { type: 'tag', enabled: true } );
				if (tag_limit && (job.tags.length > tag_limit.amount)) {
					self.appendMetaLog(job, `WARNING: Chopping ${Math.floor(job.tags.length - tag_limit.amount)} tags to adhere to limit of ` + tag_limit.amount);
					job.tags.splice( tag_limit.amount );
				}
				...
				var combo_job = Tools.mergeHashes( job, info );
				...
				if (info.serverData && Tools.isaHash(info.serverData) && job.server && self.servers[job.server]) {
					self.logJob(5, "Updating server user data: " + job.server, info.serverData);
					
					var server = self.servers[job.server];
					if (!server.userData) server.userData = {};
					Tools.mergeHashInto( server.userData, info.serverData );
	...
	deleteJob(job, callback) {
		...
		async.series([
			...
			function(callback) {
				...
				if (!job.workflow || !job.workflow.job || !job.workflow.node) return process.nextTick(callback);
				
				self.unbase.update( 'jobs', job.workflow.job, 
					function(wf_job) {
						if (!wf_job.workflow || !wf_job.workflow.jobs || !wf_job.workflow.jobs[job.workflow.node]) return false;
						var stub = Tools.findObject( wf_job.workflow.jobs[job.workflow.node], { id: job.id } );
						if (stub) {
							stub.deleted = true;
							return { workflow: wf_job.workflow };
						}
						else return false;
					},
					function(err) {
						// ignore error, workflow may have been deleted
						callback();
					}
				); // unbase.get
			},
			function(callback) {
				...
				for (var node_id in job.workflow.jobs) {
					job.workflow.jobs[node_id].forEach( function(stub) {
						if (!stub.deleted) job_ids.push( stub.id );
					} );
				}
