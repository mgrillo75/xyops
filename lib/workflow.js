...
class Workflows {
	...
	startWorkflow(job) {
		...
		details.wfJobData = {};
		if (!details.workflowData) details.workflowData = {}; // may be passed in
		...
		var data = (details.input && details.input.data) ? details.input.data : {};
	...
	runWFNode_trigger(opts) {
		...
		if (trigger.type.match(/^(schedule|single|interval)$/)) {
			event.triggers.filter( function(item) { return !!item.type.match(/^(catchup|nth|range|blackout|delay|precision|quiet|plugin)$/); } ).forEach( function(item) {
	...
	runWFNode_event(opts) {
		...
		// if we're launching a sub-workflow, it MUST have a manual trigger (so we know what node to start on)
		var trigger = Tools.findObject( event.triggers, { type: 'manual', enabled: true } );
		if (!trigger && event.workflow) {
			return this.abortJob(job, "Sub-workflow does not allow manual job runs: " + event.title);
		}
		...
		new_job.workflow.params = job.params || {};
		new_job.workflow.now = job.now;
		
		// set start node for sub-workflows
		if (event.workflow && !new_job.workflow.start) new_job.workflow.start = trigger.id;
		...
		if (node.data.expression) new_job.expression = node.data.expression;
		...
		// copy over special flags if the parent workflow job has them
		if (job.test) new_job.test = true;
		if (job.invisible) new_job.invisible = true;
		if (job.ephemeral) new_job.ephemeral = true;
		
		// blend actions and limits
		if (!new_job.actions) new_job.actions = [];
		if (!new_job.limits) new_job.limits = [];
		
		// copy over workflow data so job can share it
		new_job.workflowData = this.jobDetails[job.id].workflowData;
	...
	runWFNode_job(opts) {
		...
		if (new_job.label) { new_job.plabel = new_job.label; delete new_job.label; }
		if (new_job.icon) { new_job.picon = new_job.icon; delete new_job.icon; }
	...
	runWFNode_action(opts) {
		...
		if (node.data.type == 'suspend') {
			this.logWorkflow(job, node.id, `WARNING: Suspend actions must be wired to an event or job node.`);
			state.error = true;
			return;
		}
		...
		action.overrides = opts.overrides || {};
	...
	runWFController_multiplex(opts) {
		...
		if (conns.length > 1) {
			this.logWorkflow(job, node.id, `WARNING: Multiplex controller can only have a single output node (found ${conns.length})`);
			state.error = true;
			return;
		}
		var conn = conns[0];
		...
		if (!dest.type.match(/^(event|job)$/)) {
			this.logWorkflow(job, node.id, `WARNING: Multiplex controller can only be wired to an event or job node.`);
			state.error = true;
			return;
		}
		...
		var event = null;
		if (dest.data.event) {
			event = Tools.findObject( this.events, { id: dest.data.event } );
			if (!event) {
				// should never happen
				state.error = true;
		...
		// pull targets out of node, or use event targets if applicable
		var targets = dest.data.targets || [];
		if (!targets.length && event) targets = event.targets;
		
		// pull expression out of node, or use event expression if applicable
		var expression = dest.data.expression || '';
		if (!expression && event) expression = event.expression || '';
		...
		if (expression) try {
			server_ids = server_ids.filter( function(server_id) {
				return !!jexl.evalSync( expression, self.servers[server_id] );
			} );
		}
		catch (err) {
			state.error = true;
			return this.abortJob(job, "Failed to evaluate target expression: " + expression + ": " + err);
		}
		
		if (!server_ids.length) {
			// totally could happen
			this.logWorkflow(job, node.id, `WARNING: No servers available for multiplex controller`);
			return;
		}
	...
	runWFController_repeat(opts) {
		...
		if (conns.length > 1) {
			this.logWorkflow(job, node.id, `WARNING: Repeat controller can only have a single output node (found ${conns.length})`);
			state.error = true;
			return;
		}
		var conn = conns[0];
		...
		if (!dest.type.match(/^(event|job)$/)) {
			this.logWorkflow(job, node.id, `WARNING: Repeat controller can only be wired to an event or job node.`);
			state.error = true;
			return;
		}
	...
	runWFController_split(opts) {
		...
		// ensure split controller always has access to workflow job object and params
		if (!context.workflow) context.workflow = job;
		if (!context.params) context.params = job.params || {};
		if (!context.workflowData) context.workflowData = details.workflowData;
		
		// evaluate split expression
		var exp = node.data.split;
		var value = false;
		this.logWorkflow(job, node.id, `Split controller evaluating expression: {${exp}}`);
		
		try {
			value = jexl.evalSync( exp, context );
		}
		catch (err) {
			this.logWorkflow(job, node.id, `WARNING: Failed to evaluate split expression: {${exp}}: ${err}`);
			state.error = true;
			return;
		}
		
		if (!value) {
			this.logWorkflow(job, node.id, `WARNING: Split data path does not point to an array: {${exp}}`);
			state.error = true;
			return;
		}
		...
		if (!value.length) {
			this.logWorkflow(job, node.id, `WARNING: Split data array is empty: {${exp}}`);
			state.error = true;
			return;
		}
		...
		if (conns.length > 1) {
			this.logWorkflow(job, node.id, `WARNING: Split controller can only have a single output node (found ${conns.length})`);
			state.error = true;
			return;
		}
		var conn = conns[0];
		...
		if (!dest.type.match(/^(event|job)$/)) {
			this.logWorkflow(job, node.id, `WARNING: Split controller can only be wired to an event or job node.`);
			state.error = true;
			return;
		}
	...
	runWFController_join(opts) {
		...
		if (conns.length > 1) {
			this.logWorkflow(job, node.id, `WARNING: Join controller can only have a single output node (found ${conns.length})`);
			state.error = true;
			return;
		}
		var conn = conns[0];
	...
	runWFController_decision(opts) {
		...
		// ensure decision controller always has access to workflow job object and params
		if (!context.workflow) context.workflow = job;
		if (!context.params) context.params = job.params || {};
		if (!context.workflowData) context.workflowData = details.workflowData;
	...
	finishWorkflowJob(sub_job) {
		...
		if (sub_job.label) stub.label = sub_job.label;
		if (sub_job.icon) stub.icon = sub_job.icon;
	...
	tickWorkflow(job) {
		...
		if (!details.files) details.files = [];
		...
		// promote workflowData into data and delete orig
		Tools.mergeHashInto( details.data, details.workflowData );
		delete details.workflowData;
	...
	abortWorkflow(job, reason) {
		...
		// abort may come very early, so we may need to init some things
		if (!job.workflow.state) job.workflow.state = {};
		if (!job.workflow.jobs) job.workflow.jobs = {};
		
		// store sub-job output in details, so it isn't synced to clients every second
		var details = this.jobDetails[job.id];
		if (details && !details.wfJobData) details.wfJobData = {};
		
		this.appendMetaLog(job, "Aborting Job: " + reason);
		this.logWorkflow(job, null, "Aborting workflow: " + reason);
		
		// deactivate all controllers here
		Tools.findObjectsDeep( job.workflow.nodes, { type: 'controller' } ).forEach( function(node) {
			var state = job.workflow.state[node.id];
