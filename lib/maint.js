...
class Maintenance {
	...
	maintMinute() {
		...
		if (this.storage.engine.cache) {
			...
			delete this.stats.cache.hotKeys;
		}
	...
	archiveLogs() {
		...
		if (this.config.get('log_archive_path')) {
			...
			this.logger.archive(src_spec, dest_path, epoch, function(err) {
				...
				if (!keep) { job.finish(); return; }
				
				// find the arch base path: strip everything from the first bracket onward
				// e.g. logs/archives/[yyyy]/[mm]/[dd]/[filename]-[yyyy]-[mm]-[dd].log.gz --> logs/archives
				var arch_base_path = dest_path.replace(/\[.+$/, '').replace(/\/$/, '');
				
				self.logMaint(4, "Scanning for expired log archives in: " + arch_base_path, { keep });
				
				keep = Tools.getSecondsFromText(keep);
				if (!keep) { 
					self.logError('maint', "Failed to convert log_archive_keep to seconds: " + self.config.get('log_archive_keep'));
					job.finish(); 
					return; 
				}
				
				Tools.findFiles( arch_base_path, {
					filespec: /\.(log|gz)$/,
				...
				function(err, files) {
					...
					if (!files || !files.length) { 
						self.logMaint(4, "No expired log archives found.");
						job.finish(); 
						return; 
					}
					...
					async.eachSeries(files,
						function(file, callback) {
							self.logMaint(5, "Deleting expired log archive: " + file);
						...
						function() {
							self.logMaint(4, "Log archive deletion complete");
							job.finish();
						}
					);
				}); // Tools.findFiles
	...
	finishInternalJob(id) {
		...
		if (job.username && !job.quiet && !job.hush) {
		...
		// emit event for internal job completion
		this.emit('internalJobCompleted', job);
							...
							var finish = function() {
								...
								self.unbase.delete( opts.index, id, function(err) {
									if (err) {
										...
										if (err.code != 'NoSuchKey') self.logError('unbase', "" + err);
									}
									else {
										opts.stats.count++;
	...
	dbSearchDelete_jobs(job, stats, callback) {
		...
		if (job.input && job.input.files && job.input.files.length) {
			// include input files, but only if not attached to buckets/tickets
			var ipaths = job.input.files.filter( function(file) { return !file.bucket && !file.ticket; } ).map( function(file) { return file.path; } );
			if (ipaths.length) to_delete = to_delete.concat( ipaths );
		}
		...
		async.eachLimit(to_delete, this.storage.concurrency, 
			...
			function() {
				// recurse for workflow sub-jobs
				if ((job.type != 'workflow') || !job.workflow || !job.workflow.jobs) return callback();
				
				// collect all sub-job ids from workflow
				var job_ids = [];
				...
				if (!job_ids.length) return callback();
				
				self.logJob(6, "Deleting all workflow sub-jobs for: " + job.id, { ids: job_ids });
				...
				async.eachSeries( job_ids, function(job_id, callback) {
					self.unbase.get( 'jobs', job_id, function(err, sub_job) {
						...
						self.unbase.delete( 'jobs', job_id, function(err) {
							if (err) {
								if (err.code != 'NoSuchKey') self.logError('unbase', "" + err);
								return callback();
							}
							
							// manipulate stats so sub-job is included (and counts towards max, if set!)
							stats.count++;
							stats.total++;
							
							self.dbSearchDelete_jobs(sub_job, stats, callback);
						} ); // unbase.delete
					} ); // unbase.get
				}, callback); // eachSeries
			}
