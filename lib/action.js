...
class Actions {
	...
	runJobAction_run_event(job, action, callback) {
		...
		if (action.start_delay) {
			new_job.state = 'start_delay';
			new_job.until = Tools.timeNow(true) + action.start_delay;
		}
	...
	runJobAction_suspend(job, action, callback) {
		...
		if (action.sources && action.sources.length) {
			// consider job source before suspending
			var job_source = job.source;
			var temp_job = job;
			
			// walk out to topmost parent workflow to get actual source
			while ((job_source == 'workflow') && temp_job.workflow && temp_job.workflow.job && this.activeJobs[temp_job.workflow.job]) {
				temp_job = this.activeJobs[temp_job.workflow.job];
				job_source = temp_job.source;
			}
			
			if (!action.sources.includes(job_source)) {
				action.code = 0;
				action.description = "Skipped suspension due to job source: " + job_source;
				if (!action.details) action.details = "";
				return callback();
			}
		} // action.sources
	...
	runJobAction_label(job, action, callback) {
		// add a custom label to the job
		if (!callback) callback = noop;
		var hook_args = this.getJobHookData(job, action);
		var label = this.messageSub( String(action.label), hook_args, 'n/a' ).replace(/<[^>]*>/g, '');
		
		this.appendMetaLog(job, `Applying label to job: ${label}`);
		job.label = label;
		
		action.code = 0;
		action.description = "Successfully applied label.";
		callback();
	}
		...
		async.series([
			...
			function(callback) {
				...
				self.storage.get( bucket_path + '/data', function(err, bucket_data) {
					...
					if (Array.isArray(bucket_data)) info.input.data.items = bucket_data;
					else Tools.mergeHashInto(info.input.data, bucket_data);
	...
	runJobAction_ticket(job, action, callback) {
		...
		var ticket_template = this.config.getPath('client.new_ticket_template') || {};
		
		var ticket = {
			subject: '', // auto-generated via template
			...
			due: action.ticket_due || ticket_template.due || '',
			server: job.server || ''
		};
		
		if (ticket.due && (typeof(ticket.due) == 'string')) {
			// convert deltas like "7 days" to seconds
			ticket.due = Tools.getSecondsFromText(ticket.due);
		}
		if (ticket.due && (typeof(ticket.due) == 'number') && (ticket.due < 315360000)) {
			// appears to be an offset, so add current time and normalize
			ticket.due = Tools.normalizeTime( Tools.timeNow(true) + ticket.due, { hour: 0, min: 0, sec: 0 } );
		}
	...
	runJobAction_plugin(job, action, callback) {
		...
		// include local base url for API calls
		var base_url = this.getLocalBaseURL();
		hook_args.base_url = base_url;
		...
		child_opts.env['XYOPS_BASE_URL'] = base_url;
	...
	fireSystemHook(action, orig_data) {
		...
		if (hook) {
			...
			if (hook.url) {
				...
				this.request.json( url, data, {}, function(err, resp, data) {
					// log response
	...
	fireWebHook(id, data, callback) {
		...
		// allow placeholder subs on url (encoded)
		var url = this.messageSub( hook.url, data, data._fallback );
		...
		if (hook.headers && !Array.isArray(hook.headers)) {
			// preventing crash re: issue # 238, will remove later
			return callback( new Error("Web Hook has invalid headers: " + id) );
		}
		...
		this.logAction(9, "Sending web hook request: " + url);
		...
		this.request.request( url, opts, function(err, resp, data, perf) {
			...
			if (Tools.numKeys(opts.headers)) {
				...
				for (var key in opts.headers) {
					details += key + ": " + scrubSecrets(opts.headers[key]) + "\n";
				}
				details += "```\n";
			}
			...
			if (opts.data && opts.data.length) {
				details += "\n**Request Body:**\n\n```\n";
				details += scrubSecrets(opts.data.toString().trim()) + "\n```\n";
			}
			...
			if (resp && resp.rawHeaders) {
				...
				if (data && data.length) {
					details += "\n**Response Body:**\n\n```\n";
					details += scrubSecrets(data.toString().trim()) + "\n```\n";
				}
